...
SET
  ...
  link = '',
  remark = 'API Key 管理',
  parent_id = (SELECT id FROM sys_menu WHERE name = 'System' LIMIT 1),
...
SELECT
  ...
  '',
  NULL,
  (SELECT id FROM sys_menu WHERE name = 'PluginApiKey' LIMIT 1),
  CURRENT_TIMESTAMP,
  CURRENT_TIMESTAMP
WHERE NOT EXISTS (
...
SET
  ...
  link = '',
  remark = NULL,
  parent_id = (SELECT id FROM sys_menu WHERE name = 'PluginApiKey' LIMIT 1),
  updated_time = CURRENT_TIMESTAMP
